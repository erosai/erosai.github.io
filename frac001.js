...
function setup() {
    cnv = createCanvas(2048, 1080,WEBGL);
...
function draw() {    
  ...
  test.setUniform("iTime", 10000 + millis()*0.0001  );
	test.setUniform('iMouse',[millis()*0.02,millis()*0.02]);
...
vec2 mouse=iMouse/iResolution;
    ...
    color = mix(color,vec3(1.856,.5*(1.0+sin(1.5+.2*sin(iTime))),0.164706),r.x+length(q));
    color = mix(color,vec3(1.5*(1.0-sin(.1*iTime)),.2+.2*(1.0+tan(0.5+.3*iTime)),1.0),length(r+q));
